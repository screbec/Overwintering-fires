...
covering northewestern North America
MODSCAG data should be in original folder structure and with original file names: modscag-historic/year/doy/
Output pt 1: Tiles of the first snow-free day of the season (beginning of spring)

processing consistes of three parts:
- computing the first snow-free day (in this script)
- merging, reprojecting and clipping tiles to the area of interest (can be done with any software)
- computing regional means for each year while excluding NA pixels and burned pixels (in this script)
...
def firstDay(imgfile):
    ...
    arr = band.ReadAsArray().astype(np.int)
...
def commonPixels(img1, img2, res, pxloc = True):
...
for tile in ['h10v02', 'h11v02', 'h12v02', 'h13v02']:       # for each tile
    ...
    for y in range(2000,2019):
        ...
        doy_arr = [firstDay(file) for file in imgs]
...
#%% this only created the first snow-free day tiles -----------------------------
...
xmin_ba, xmin_snow, ymin_ba, ymin_snow, xn, yn = commonPixels(BA_img, snow_image, 500)
...
for bnd in range(BA_img.RasterCount)[:-1]:
    ...
    arr = band.ReadAsArray(xmin_ba, ymin_ba, xn, yn)
    try:
        mask = mask & (arr == 0)
    except:
        mask = (arr == 0)
...
for bnd in range(snow_image.RasterCount):
    ...
    arr = band.ReadAsArray(xmin_snow, ymin_snow, xn, yn)
    mask = mask & (arr != 0)
...
for bnd in range(snow_image.RasterCount):
    ...
    band = snow_image.GetRasterBand(bnd + 1)
    arr = band.ReadAsArray(xmin_snow, ymin_snow, xn, yn)
    out.append(np.nanmean(arr[mask]))
    res.append(out)
snow_image = None

df = pd.DataFrame(res, columns = ['Year','ldos'])
df.to_csv(outfile)
